...
enum ReminderEngine {
    static func reconcile(_ snapshot: inout PetPlanifySnapshot, now: Date = .now) {
        ...
        let suppressedSourceKeys = Set(snapshot.trash.compactMap { $0.kind == .reminder ? $0.sourceKey : nil })
        ...
        for records in vaccines.values {
            if let value = records.max(by: { $0.dateAdministered < $1.dateAdministered }), let date = value.nextDueDate {
                sources.append(("vaccine.\(value.id)", value.id, String(localized: "Vacuna: \(value.name)"), date, value.notes ?? ""))
            }
        }
        for kind in DewormingKind.allCases {
            if let value = snapshot.health.dewormings.filter({ $0.kind == kind }).max(by: { $0.applicationDate < $1.applicationDate }), let date = value.nextDueDate {
                sources.append(("deworming.\(value.id)", value.id, String(localized: "Desparasitación: \(kind.shortTitle)"), date, value.productName ?? ""))
            }
        }
        for medication in snapshot.health.medications {
            let key = "medication.\(medication.id)"
